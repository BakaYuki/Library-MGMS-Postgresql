...
def return_book():
    ...
    if st.button("Show Books Issued"):
        ...
        if check:
            ...
            try:
                cur.execute(
                    "SELECT books.book_id, books.title, books.author, books.genre FROM issued JOIN books ON issued.book_id = books.book_id WHERE issued.student_id = %s",
                    (student_id,)
                )
                ...
                if issued_books:
                    ...
                    for book_id, title, author, genre in issued_books:
                        st.write(f"ID: {book_id}, Title: {title}, Author: {author}, Genre: {genre}")
